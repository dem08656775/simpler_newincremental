...
Vue.createApp({
  ...
  methods: {
    ...
    calcgncost(){
      for(let i=0;i<8;i++){
        let p = i === 0 ? this.player.generatorsBought[0] : this.player.generatorsBought[i].add(i).mul(i + 1)
    ...
    update() {
      ...
      if(!this.player.onchallenge && this.activechallengebonuses.includes(14) && this.autolevel){
        if(this.player.money.greaterThanOrEqualTo('1e21')){
      ...
      this.player.tickspeed = 100 / this.player.accelerators[0].add(10).mul(amult).log10()
    ...
    importsave(){
      ...
      input = atob(input)
      localStorage.setItem("playerStoredoldold",input)
      this.load()
    },
    save() {
      console.log(JSON.stringify(this.player))
      console.log(JSON.stringify(this.activechallengebonuses))
      localStorage.setItem("playerStoredold", JSON.stringify(this.player));
    },
    load() {
      if (!localStorage.getItem("playerStoredold")) return
      let saveData = JSON.parse(localStorage.getItem("playerStoredold"));
    ...
    buylevelitems(index){
      ...
      if(this.player.level.lessThan(cost) || this.player.levelitems[index]>=6){
        return;
      }
    ...
    resetRank(){
      let gainrank = new Decimal(this.player.money.log10()).div(42).pow_base(2).round()
    ...
    startChallenge(){
      ...
      if (confirm('挑戦を開始しますか？現在のポイントや発生器、時間加速器は失われます。')) {
        ...
        if(this.player.challenges.includes(3)){
          for(let i=0;i<8;i++){
            this.player.generatorsMode[i] = Math.trunc(i/2)
          }
        }
      }
    },
